...
\lnTitlePage{8}{10}{2H23zowrLMg}

\lnQuote
  {rita-mulcahy}
  {When you have done risk management on a project,
    your project will go smoother and faster, with significantly
    fewer complications, because avoidable problems
    were solved before they happened.}
  {rita2009pmp}
...
\pmbaQuestion
  {You just found out that one of your key programmers posted
    his resume on a job board. What do you do \hs{immediately}?}
  {Nothing}
  {Update risk list}
  {Post a vacancy}
  {Offer him a raise}
...
\pmbaQuestion
  {What is the cost of a risk, if \hs{probability} is 0.8,
    \hs{impact} is 0.7, and \hs{effect} is \pounds1,000?}
  {\pounds1,500}
  {\pounds560}
  {\pounds800}
  {\pounds700}
...
\pmbaQuestion
  {What is the name of a risk with a \hs{negative value}
    of the \hs{impact}?}
  {Opportunity}
  {Residual Risk}
  {Threat}
...
\pmbaQuestion
  {What is the relation of elements in the \hs{cause-risk-effect} format?}
  {Many-one-many}
  {Many-many-many}
  {Many-one-one}
  {One-one-one}
...
\pmbaQuestion
  {At a technical meeting, a programmer says: "I think the use
    of this framework \hs{will} lead to problems in the future".
    What do you answer?}
...
\pmbaQuestion
  {A new architect replaces the previous one and tells you
    that there is a \hs{serious} security flaw in the system.
    What do you do?}
  {Ask him to \hs{fix} it immediately}
  {Ask him to \hs{not disclose} this information to anyone}
  {Ask him to \hs{estimate} the risk of attack}
  {Nothing}
  {identification}

\pmbaQuestion
  {The estimated duration of the project is 20 months.
    There is a \hs{3x9} risk that a few key engineers
    may leave the company soon. If this happens, the project
    will be delayed by another 15 months. What project duration
    should you report now to the customer?}
  {\(20 + 15 \times 0.3 = 24.5\)}
...
\pmbaQuestion
  {There is a \hs{risk} that the server will break.
    If this happens, a new one will cost another \$5,000.
    How to prepare the customer for this?}
...
\plush{
  \pptBanner{Homework:}
  ``\hs{Risk Register} is a document in which the results of risk analysis and risk response planning
