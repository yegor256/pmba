% SPDX-FileCopyrightText: Copyright (c) 2023-2026 Yegor Bugayenko
% SPDX-License-Identifier: MIT

\documentclass{article}
\usepackage{../pmba}
\newcommand*\thetitle{Quality Management}
\begin{document}

\lnTitlePage{5}{10}{1ltOS2He890}

\plush{
\pptBanner{Management \st{Triangle} Rectangle}\par
\begin{tikzpicture}[node distance=5em,
  every path/.style={draw=black, line width=.1em},
  every node/.style={font={\Large\ttfamily}, draw=black, rectangle, outer sep=.5em}]
\node[draw=none] (center) {};
\node[above of=center] (scope) {Scope};
\node[below of=center] (quality) {Quality};
\node[right of=center] (time) {Time};
\node[left of=center] (cost) {Cost};
\path (scope) -- (time);
\path (time) -- (cost);
\path (cost) -- (scope);
\path (quality) -- (time);
\path (quality) -- (cost);
\path (quality) -- (scope);
\end{tikzpicture}
}

\pmbaQuestion
  {ISO 9000 \hs{defines} quality as ...}
  {the ultimate objective of any project}
  {the indicator of customer satisfaction}
  {the amount of defects that the customer experiences in the product}
  {the degree to which a set of characteristics fulfil requirements}
  {iso}

\pmbaQuestion
  {The architect tells you that 15 bugs can't be fixed in the current release
    and will be \hs{shipped} to the customer. What do you say?}
  {``Cancel the release!''}
  {``Don't release until they are all fixed!''}
  {``Don't tell the customer, ship it anyway.''}
  {``How much trouble may they cause?''}
  {coq}

\pmbaQuestion
  {You just hired a ``\hs{QA} Expert.'' What task would you give him?}
  {Help programmers make our app bug-free}
  {Test our mobile app and make sure it works well}
  {Find three bugs in our app}
  {Make sure every bug is in JIRA}
  {qa}

\pmbaQuestion
  {You want to increase \hs{quality of product},
    which \hs{metric} would you control first?}
  {Git commits per day}
  {Customer complaints per day}
  {CI failures per day}
  {New bugs per day}
  {qc}

\pmbaQuestion
  {You recruit an external team of testers, how would you \hs{pay} them?}
  {\$100 per hour}
  {\$100 per feature they test}
  {\$100 per test case they execute}
  {\$100 per bug they find}
  {testers}

\pmbaQuestion
  {Customer complains that the quality is low:
    too many bugs in each new release.
    How can you find out the \hs{root cause}?}
  {Five Whys}
  {Plan-Do-Check-Act}
  {Six Sigma}
  {Fish-bone Analysis}
  {root-cause}

\pmbaQuestion
  {The biggest \hs{enemy} of quality is...}
  {Bureaucracy}
  {Bugs}
  {Negligence}
  {Enthusiasm}
  {ingredients}

\pmbaQuestion
  {There are four bugs reported,
    which one would you prioritize as more \hs{important} than others?}
  {A flow is missed in the Use Case}
  {A class has too many methods}
  {A unit test breaks the build}
  {A security loophole discovered in production server}
  {priorities}

\plush{
  \pptBanner{Homework:}
  ``\hs{Quality Management Plan} describes how quality policies will be implemented and how the project management team plans to meet the quality requirements set for the project''
  --- PMBOK5
  }

\plush{
  \pptBanner{Read this:}
  \href{https://www.youtube.com/watch?v=jZitXMQaXvE}{Quality Assurance vs. Testing} at QAFest'19\par
  \href{https://www.yegor256.com/2022/07/05/safety-net.html}{Automated Tests Are the Safety Net that Saves You}\par
  \href{https://www.yegor256.com/2017/05/23/unlimited-number-of-bugs.html}{Any Program Has an Unlimited Number of Bugs} (2017)\par
  \href{https://www.yegor256.com/2015/09/10/testing-exit-criteria.html}{When Do You Stop Testing?} (2015)\par
  \href{https://www.yegor256.com/2014/08/22/art-of-software-testing.html}{The Art of Software Testing by Glenford Myers} (2014)\par
  \href{https://www.yegor256.com/2017/12/26/software-quality-formula.html}{The Formula for Software Quality} (2017)\par
  \href{https://www.yegor256.com/2019/12/24/altruism-kills.html}{Altruism Kills!} (2019)\par
  \href{https://www.yegor256.com/2015/06/18/good-programmers-bug-free.html}{Good Programmers Write Bug-Free Code, Don't They?} (2015)\par
  }

\end{document}
