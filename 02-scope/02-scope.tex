% SPDX-FileCopyrightText: Copyright (c) 2023-2026 Yegor Bugayenko
% SPDX-License-Identifier: MIT

\documentclass{article}
\usepackage{../pmba}
\newcommand*\thetitle{Scope Management}
\begin{document}

\lnTitlePage{2}{10}{7G0dE7Zxew4}

\plush{
  \pptBanner{Management Triangle}\par
  \begin{tikzpicture}[graph, node distance=5em]
  \node (scope) {Scope};
  \node[below right of=scope] (time) {Time};
  \node[below left of=scope] (cost) {Cost};
  \path (scope) -- (time);
  \path (time) -- (cost);
  \path (cost) -- (scope);
  \end{tikzpicture}\par
  Your responsibility, as a project manager, is to \hs{control} that all three elements of the
  triangle are \hs{in sync}: when one of them changes, you have to change the other two. If you don't
  know how much the project will cost, how long it will take, or how much functionality
  it will implement---you are not a project manager. Maybe a leader, a mentor, or ... a scapegoat.
  }

\pmbaQuestion
  {Even though it was not required by the customer, a \hs{programmer} suggests implementing an additional feature in the product, because it is obvious that users will love it; what do you say, as a PM?}
  {``Definitely not!''}
  {``Maybe we should discuss with the customer first?''}
  {``Only if it doesn't delay all other features''}
  {``Sure, customer first!''}
  {gold-platting}

\pmbaQuestion
  {Which definition of a task for a \hs{\st{programmer} agent} sounds perfect for you?}
  {``Make sure \ff{FooTest\#test15()} passes in the \ff{master} branch!''}
  {``Fix the build, make sure it's green!''}
  {``Together with the team, resolve the problem ASAP!''}
  {``Make sure the customer is happy!''}
  {d-o-d}

\pmbaQuestion
  {Which one is the right formulation of a functionality in a Use Case?}
  {User \hs{can} download a picture}
  {User \hs{downloads} a picture}
  {User \hs{will} download a picture}
  {User \hs{should} download a picture}
  {use-case}

\pmbaQuestion
  {A customer asks you how much \hs{work} is left to be done. Where do you find this information?}
  {You ask your team}
  {Use Cases}
  {Backlog}
  {Traceability Matrix}
  {scope-control}

\pmbaQuestion
  {Which one \hs{isn't} a Non-Functional Requirement (NFR)?}
  {When ``Send'' is clicked, email must be sent in less than 500ms}
  {The ``Settings'' page must be intuitively easy to use}
  {A picture of 100Kb downloads in less than 1.5s}
  {In case of a security breach at any web server, user passwords won't leak}
  {use-case}

\pmbaQuestion
  {After six months of hard work, your team releases the product to customer's servers. The customer says: ``This is not what I wanted :('' Whose fault is it?}
  {The \hs{project manager} didn't validate it earlier}
  {The \hs{customer} didn't explain what they wanted}
  {\hs{Testers} didn't verify it earlier}
  {It's \hs{nobody's} fault! We don't blame! We learn and improve!}
  {V\&V}

\pmbaQuestion
  {A project of \hs{one year} and \hs{five programmers} can be decomposed into how many Work Packages?}
  {Nobody knows}
  {Seven}
  {25 sprints, 5 coders, 1 WP per week \(\to\) 250 WPs}
  {Hundreds}
  {use-case}

\pmbaQuestion
  {Which estimate of project scope is the most reasonable?}
  {100 \hs{Function Points} implemented}
  {500 \hs{Pull Requests} merged}
  {50,000 \hs{Lines of Code} written}
  {10,000 \hs{hours} of hard work, including weekends}
  {estimate}

\plush{
  \pptBanner{Homework:}
  ``A \hs{Vision} document defines the high-level scope and purpose of a program,
  product, or project. A clear statement of the problem, proposed solution,
  and the high-level features of a product helps establish expectations and reduce risks.''
  --- IBM Engineering Lifecycle Management
}

\plush{
  \pptBanner{Read this:}\par
  \href{https://www.youtube.com/watch?v=RglMmJb0PZ4}{SSD 2/16: Lecture about Requirements Engineering} (2022)\par
  Wikipedia:
  \href{https://en.wikipedia.org/wiki/COSMIC_functional_size_measurement}{COSMIC},
  \href{https://en.wikipedia.org/wiki/Function_point}{Function Point},
  \href{https://en.wikipedia.org/wiki/MoSCoW_method}{MoSCoW method}\par
  \nospell{Karl Wiegers} et al., \hs{Software Requirements} (1999)\par
  \nospell{Alistair Cockburn}, \hs{Writing Effective Use Cases} (1999)\par
  \href{https://www.yegor256.com/2014/04/26/incremental-requirements-with-requs.html}{Incremental Requirements With Requs} (2014)\par
  \href{https://www.yegor256.com/2015/11/10/ten-mistakes-in-specs.html}{10 Typical Mistakes in Specs} (2015)\par
  \href{https://www.yegor256.com/2014/10/20/how-we-write-product-vision.html}{How We Write a Product Vision} (2014)\par
}

\end{document}
